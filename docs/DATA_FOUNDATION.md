...
Version 2 now shares the educational CRM mode across all five modules. Unsupported source metrics return unavailable. A separately generated planning universe supplies quotas, snapshots, revenue schedules, recurring ledgers and payment examples, each explicitly labeled synthetic. See README_OPERATING_MODEL.md for the current implementation.
...
The source's original synthetic generator is unavailable; only the transformations from the preserved CSVs are reproducible. The previous audit remains a record of the earlier HTML review, and remaining evidence and original-source gaps are tracked in IMPLEMENTATION_STATUS.md.
