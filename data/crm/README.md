...
All five version-2 modules share an observed CRM mode calculated from this package. Metrics absent from the educational source remain unavailable. A separate, clearly labeled planning universe provides generated snapshots, quotas and non-CRM ledgers; its documentation is in `docs/README_OPERATING_MODEL.md`. The preserved CRM records are never relabeled as observed ARR, payment volume or historical forecast calls.
